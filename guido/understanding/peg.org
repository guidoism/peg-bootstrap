...
_               <- sp _   
                  /    .
rule              <- n: name _ '<-' _ body: choice '.' _ -> (format(['State parse_', n, '(str input, int pos) { State state = { .pos=pos }; Stack stack = {0}; ',
         body, ' return state; }']))
 .
...
nonterminal       <- n: name _ -> (`state = parse_${n}(input, state.pos);\n`)
 .
labeled           <- label: name _ ':' _ value: term -> (format([value, ' if (valid) { var("', label, '", state.val); }']))
 .
...
choice            <- a: sequence '/' _  b: choice -> (format(['push(state); ',
	      ...
	      '} else { pop(); }']))
                  ...
                  / sequence   .
negation          <- '!' _ t: term -> (format([' push(state);', t, 
