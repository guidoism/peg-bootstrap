...
function-scoped `var`-style variables to a function-scoped
dictionary called `vars` which will be used by the `format`
function.
...
_               <- sp _   
                  /    .
rule              <- n: name _ '<-' _ body: choice '.' _ -> (format(['function parse_', n, '(input, pos) { let state = { pos: pos }; let stack = []; ',
         'let vars = {}',
         body, ' return state; }']))
 .
...
nonterminal       <- n: name _ -> (`state = parse_${n}(input, state.pos);\n`)
 .
labeled           <- label: name _ ':' _ value: term -> (format([value, ' if (state) { vars["', label, '"] = state.val; }']))
 .
sequence          <- foo: term  bar: sequence -> (format([foo, ' if (valid) { ', bar, ' } ']))
...
negation          <- '!' _ t: term -> (format([' stack.push(state);', t, 
 ...
 .
result_expression <- '->' _ result: expr _ -> (format(['if (valid) { state.val = ', result, '; }']))
 .
...
#+BEGIN_SRC js
(format(['function parse_', n, '(input, pos) { let state = { pos: pos }; let stack = []; ',
         'let vars = {}',
         body, ' return state; }']))
#+END_SRC
...
#+BEGIN_SRC js
(format([value, ' if (state) { vars["', label, '"] = state.val; }']))
#+END_SRC
...
#+BEGIN_SRC js
(format(['if (valid) { state.val = ', result, '; }']))
#+END_SRC
