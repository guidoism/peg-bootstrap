                ...
                if (state) {
                    ...
                    if (state) {
                        ...
                        if (state) {
                            state = parse__(input, state.pos);
                            if (state) {
                                if (state) state.val = (format(['function parse_', n, '(input, pos) { var state = { pos: pos }; var stack = []; ',
                                    body, ' return state; }'
                                ]));
                            }
                        }
...
function parse_grammar(input, pos) {
    ...
    if (!state) {
        ...
        if (state) {
            ...
            if (state) {
                if (state) state.val = ('// This is from a template in peg.org:\n' + r +
...
function parse_nonterminal(input, pos) {
    ...
    if (state) {
        state = parse__(input, state.pos);
        if (state) {
            if (state) state.val = (format(['state = parse_', n, '(input, state.pos);']));
        }
    }
        ...
        if (state) {
            ...
            if (state) {
                ...
                if (state) {
                    ...
                    if (state) {
                        if (state) state.val = (format([value, ' if (state) { var ', label, ' = state.val; }']));
                    }
                }
            }
        }
...
function parse_sequence(input, pos) {
    ...
    if (state) {
        ...
        if (state) {
            if (state) state.val = (format([foo, ' if (state) { ', bar, ' } ']));
        }
    }
    if (!state) {
        state = stack.pop();
    ...
    if (state) {
        ...
        if (state) {
            ...
            if (state) {
                state = parse__(input, state.pos);
                if (state) {
                    if (state) state.val = (format(['state = literal(input, state.pos, \'', s, '\');']));
                }
            }
        }
    }
...
function parse_negation(input, pos) {
    ...
    if (state) {
        ...
        if (state) {
            ...
            if (state) {
                if (state) state.val = (format([' stack.push(state);', t,
    ...
    if (state) {
        ...
        if (state) {
            ...
            if (state) {
                state = parse__(input, state.pos);
                if (state) {
                    if (state) state.val = (format(['if (state) { state.val = ', result, '; }']));
                }
            }
        }
    }
