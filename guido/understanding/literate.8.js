...
function parse_grammar(input, pos) {
    ...
    if (state) {
        ...
        if (state) {
            ...
            if (state) {
                if (state) state.val = (format([vars["r"], " ", vars["g"]]));
            }
        }
    }
    if (!state) {
        ...
        if (state) {
            ...
            if (state) {
                if (state) state.val = (format(["// This is from a template in peg.org:\n", vars["r"],
...
function parse_name(input, pos) {
    ...
    if (state) {
        ...
        if (state) {
            if (state) state.val = (format([vars["c"], vars["n"]]));
        }
    }
    if (!state) {
        state = stack.pop();
...
function parse_nonterminal(input, pos) {
    ...
    if (state) {
        state = parse__(input, state.pos);
        if (state) {
            if (state) state.val = (format(["state = parse_", vars["n"], "(input, state.pos);"]));
        }
    }
    return state;
}
        ...
        if (state) {
            ...
            if (state) {
                ...
                if (state) {
                    ...
                    if (state) {
                        if (state) state.val = (format([vars["value"],
                            " if (state) { vars[\"",
                            vars["label"],
                            "\"] = state.val; }"
                        ]));
...
function parse_sequence(input, pos) {
    ...
    if (state) {
        ...
        if (state) {
            if (state) state.val = (format([vars["foo"],
                " if (state) { ",
                vars["bar"],
                " } "
            ]));
    ...
    if (state) {
        ...
        if (state) {
            ...
            if (state) {
                state = parse__(input, state.pos);
                if (state) {
                    if (state) state.val = (format(["state = literal(input, state.pos, \"",
                        vars["s"],
                        "\");"
                    ]));
                }
    ...
    if (state) {
        ...
        if (state) {
            ...
            if (state) {
                ...
                if (state) {
                    if (state) state.val = (format([vars["c"], vars["s"]]));
                }
            }
        }
    }
    if (!state) {
        ...
        if (state) {
            ...
            if (state) {
                ...
                if (state) {
                    if (state) state.val = (format([vars["b"], vars["c"], vars["s"]]));
                }
            }
        }
        if (!state) {
    ...
    if (state) {
        ...
        if (state) {
            ...
            if (state) {
                state = parse__(input, state.pos);
                if (state) {
                    if (state) state.val = (format(["if (state) { state.val = ",
                        vars["result"],
                        "; }"
                    ]));
                }
    ...
    if (state) {
        ...
        if (state) {
            ...
            if (state) {
                state = literal(input, state.pos, ")");
                if (state) {
                    if (state) state.val = (format(["(", vars["e"], ")"]));
                }
            }
        }
    }
...
function parse_exprcontents(input, pos) {
    ...
    if (state) {
        ...
        if (state) {
            if (state) state.val = (format([vars["c"], vars["e"]]));
        }
    }
    if (!state) {
        state = stack.pop();
        ...
        if (state) {
            ...
            if (state) {
                ...
                if (state) {
                    state = parse__(input, state.pos);
                    if (state) {
                        if (state) state.val = (format([vars["body"]]));
                    }
                }
            }
        }
